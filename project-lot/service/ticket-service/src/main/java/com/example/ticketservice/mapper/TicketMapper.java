...
public interface TicketMapper extends BaseMapper<Tickets> {
    ...
    @Select("""
            ...
            LEFT JOIN tickets t ON DATE(t.created_at) = days.date
            GROUP BY days.date
            ORDER BY days.date
            """)
    ...
    @Select("""
    SELECT ROUND(AVG(TIMESTAMPDIFF(HOUR,start_time,end_time)),2) AS avgTime, COUNT(type) AS num,`type`
    FROM
    tickets
    WHERE 
    status = 'completed' AND created_at >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)
    GROUP BY `type`;
    """)
    List<Map<String, Object>> weekCompletedStatus();
} 